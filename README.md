# Сервер мессенджера Aegis

Высокопроизводительный TCP-сервер мессенджера с собственным бинарным протоколом и шифрованием.
...
- **ProtocolError**: Выбрасывается при нарушениях протокола
- **CryptoError**: Выбрасывается при ошибках криптографии
- **TransportError**: Выбрасывается при проблемах сети/транспорта

#### Логирование

- **ILogger**: Интерфейс для структурированного логирования с уровнями Debug, Info, Warning, Error
...
- **Encode(Message, Span<byte>)**: Сериализует сообщение в байтовый буфер
- **Decode(ReadOnlySpan<byte>)**: Десериализует сообщение из байтового буфера
...
- **Событийно-ориентированный**: OnClientConnected, OnClientDisconnected, OnMessageReceived
...
- **Корректное завершение**: Обработка Ctrl+C
...
- Сообщество: [ссылка на сообщество]
