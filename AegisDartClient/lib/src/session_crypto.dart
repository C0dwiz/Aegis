...
class AegisHandshakeVerifier {
  ...
  static Uint8List _buildTranscript(
      Uint8List serverPublicKey, Uint8List clientPublicKey) {
...
class AegisSecureProtocolV2 {
  ...
  static Future<AegisV2SessionKeys> deriveSessionKeys({
    ...
    final salt = Uint8List(clientNonce.length + serverNonce.length)
      ...
      ..setRange(clientNonce.length, clientNonce.length + serverNonce.length,
          serverNonce);

    final hsSecret = await _deriveHkdf(
      keyMaterial: sharedSecret,
      salt: salt,
      info: Uint8List.fromList(utf8.encode('aegis-v2/hs')),
    );

    final c2s = await _deriveHkdf(
      keyMaterial: hsSecret,
      salt: Uint8List(0),
  ...
  static Future<Uint8List> computeClientFinishProof({
    ...
    final material = Uint8List(transcriptHash.length + 6)
      ..setRange(0, transcriptHash.length, transcriptHash)
      ..setRange(transcriptHash.length, transcriptHash.length + 6,
          ascii.encode('finish'));
    final mac =
        await _hmac.calculateMac(material, secretKey: SecretKey(ackKey));
...
class AegisSessionCrypto {
  ...
  Uint8List _buildHeaderBytes(Message message) {
    ...
    bd.setUint16(7, message.type.value, Endian.big);
    bd.setUint64(9, message.sequenceId, Endian.big);
    bd.setUint32(17, message.payloadLength, Endian.big);
    return header;
  }
}
